...
import { buildPdfCompressFormData } from "./pdfCompressPayload";
...
const PdfCompressPanel = forwardRef<PdfCompressHandle, PdfCompressPanelProps>(
  function PdfCompressPanel({ selectedFileName, onValidityChange, onFieldsDirty }, ref) {
    ...
    }, [onValidityChange, selectedFileName]);

    useEffect(() => {
      onFieldsDirty();
    }, [onFieldsDirty, outputName, pdfPassword, responseMode]);
    ...
    return (
      ...
      >
        <Box sx={{ flex: 1, minHeight: 0, overflow: "auto", pb: 1 }}>
          <SettingsAccordion id="pdf-compress-source" title="Source" defaultExpanded>
            <Typography component="span" sx={fieldLabelSx}>
              PDF password
            </Typography>
            <SettingsOutlinedField
              id="pdf-compress-password"
              type="password"
...
PdfCompressPanel.displayName = "PdfCompressPanel";

export default PdfCompressPanel;
